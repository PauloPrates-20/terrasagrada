import { ConsumableItem, ConsumableSet } from '../lib/definitions';
import { FaPlusSquare, FaMinusSquare } from 'react-icons/fa';
import { useState } from 'react';
import Consumable from './Consumable';

export default function ConsumableContent({ dataSet, clickHandler }: { dataSet: ConsumableSet; clickHandler: (item: ConsumableItem) => void }) {
      ...
      <div className={`w-full overflow-hidden ${expanded ? 'max-h-fit' : 'max-h-0'}`}>
        <ul>
          {dataSet.items.map((item, index) => (
            <Consumable item={item} rarity={dataSet.rarity} key={index} clickHandler={clickHandler} />
          ))}
        </ul>
      </div>
    </div>
