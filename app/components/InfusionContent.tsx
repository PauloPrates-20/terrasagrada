import { ArtificerInfusion, InfusionSet } from '../lib/definitions';
import { FaPlusSquare, FaMinusSquare } from 'react-icons/fa';
import { useState } from 'react';
import Infusion from './Infusion';

export default function InfusionContent({ dataSet, clickHandler }: { dataSet: InfusionSet; clickHandler: (item: ArtificerInfusion) => void }) {
      ...
      <div className={`w-full overflow-hidden ${expanded ? 'max-h-fit' : 'max-h-0'}`}>
        <ul>
          {dataSet.items.map((item, index) => (
            <Infusion item={item} rarity={dataSet.rarity} key={index} clickHandler={clickHandler} />
          ))}
        </ul>
      </div>
    </div>
